...
import no.eksempel.kafka.connector.sources.github.model.Issue;
import no.eksempel.kafka.connector.sources.github.utils.DateUtils;
...
import static no.eksempel.kafka.connector.sources.github.GitHubSchemas.*;
...
public class GitHubSourceTask extends SourceTask {
    ...
    public List<SourceRecord> poll() throws InterruptedException {
        ...
        for (Object obj : issues) {
            var issue = Issue.fromJson((JSONObject) obj);
            var sourceRecord = generateSourceRecord(issue);
    ...
    private Struct buildRecordValue(Issue issue){
        ...
        var valueStruct = new Struct(VALUE_SCHEMA)
        ...
        // User is mandatory
        var user = issue.getUser();
        var userStruct = new Struct(USER_SCHEMA)
        ...
        var pullRequest = issue.getPullRequest();
        if (pullRequest != null) {
            var prStruct = new Struct(PR_SCHEMA)
